...
class Drone {
  constructor({
    ...
    if (Drone.isDevelopment) {
      this.minecart = initial_location.getWorld().spawn(initial_location, Java.type('org.bukkit.entity.minecart.PoweredMinecart').class); // EntityType.MINECART);
  ...
  async move(amount, direction) {
    ...
    // let distance = old_location.distance(this.state.location);
    // if (distance > amount) {
    //   console.log(`I don't think this is correct...`);
    //   console.log(`old_location.toString():`, old_location.toString());
    //   console.log(`this.state.location.toString():`, this.state.location.toString());
    //   console.log(`this.state.direction.toString():`, this.state.direction.toString())
    //   console.log(`new_direction.toString():`, new_direction.toString())
  ...
  async peekBlock(amount, direction) {
    ...
    if (Drone.isDevelopment === true) {
      let peek_minecart = peek_location.getWorld().spawn(block_center(peek_location), Java.type('org.bukkit.entity.minecart.PoweredMinecart').class); // EntityType.MINECART);
  ...
  async *cuboid(steps) {
    ...
    if (current == null) {
      ...
      let direction = direction_fn(this.state.direction.clone())
...
Drone.UP = (vector) => new Vector(0, 1, 0);
Drone.DOWN = (vector) => new Vector(0, -1, 0);
